...
export default async function handler(
    ...
    if (!userInvitationToken) {
        res.status(StatusCodes.BAD_REQUEST).json({ message: `No user invitation token found with ${token}!` });
        return;
    }

    if (userInvitationToken.isArchived || userInvitationToken.isObsolete || userInvitationToken.expiryDate < new Date()) {
        res.status(StatusCodes.BAD_REQUEST).json({ message: `User invitation token is obsolete!` });
        return;
    }
    ...
    if (!organisation) {
        res.status(StatusCodes.BAD_REQUEST).json({ message: `No organisation found with id ${userInvitationToken.orgId}!` });
        return;
    }
